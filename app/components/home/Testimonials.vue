<template>
  <div>
    ...
    <UPageColumns>
      ...
      >
        <template #footer>
          <UUser :name="review.name" :description="review.subtitle" :avatar="{src:review.profileImage , alt:`${review.name}'s profile images'`}" size="xl" />
        </template>
      </UPageCard>
    </UPageColumns>
  </div>
...
const { data:reviews } = useFetch<IReview[]>("/api/v1/home/reviews");
